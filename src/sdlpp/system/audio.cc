...
namespace neutrino::sdl {
	void mixer::open(unsigned channels, std::size_t chunk_size) {
		SAFE_SDL_CALL(Mix_OpenAudio, MIX_DEFAULT_FREQUENCY, MIX_DEFAULT_FORMAT, static_cast<int>(channels),
		              static_cast<int>(chunk_size));
	}

	void mixer::open(unsigned freq, format f, unsigned channels, std::size_t chunk_size) {
		SAFE_SDL_CALL(Mix_OpenAudio, freq, static_cast<int>(f), static_cast<int>(channels),
		              static_cast<int>(chunk_size));
	}
	...
	void
	mixer::play(audio_channel_id_t chan, unsigned int loops, std::chrono::milliseconds ticks,
	...
	bool
	mixer::play_on_first_available_channel(unsigned int loops, std::chrono::milliseconds ticks,
	                                       const object <Mix_Chunk>& effect) {
		return -1 != Mix_PlayChannelTimed(-1, effect.const_handle(), static_cast <int>(loops),
		                                  static_cast <int>(ticks.count()));
	}
	...
	void
	mixer::fade_in(audio_channel_id_t chan, unsigned int loops, std::chrono::milliseconds ticks,
	               const object <Mix_Chunk>& effect) {
	...
	void mixer::fade_in_inf(audio_channel_id_t chan, std::chrono::milliseconds ticks,
	                        const object <Mix_Chunk>& effect) {
		SAFE_SDL_CALL(Mix_FadeInChannel, static_cast<int>(chan.value_of ()),
		              effect.const_handle (), -1, static_cast<int>(ticks.count ()));
	}
	...
	bool
	mixer::fade_in_on_first_available_channel(unsigned int loops, std::chrono::milliseconds ticks,
	                                          const object <Mix_Chunk>& effect) {
		return -1 != Mix_FadeInChannel(-1, effect.const_handle(), static_cast <int>(loops),
		                               static_cast <int>(ticks.count()));
	}
	...
	bool mixer::fade_in_on_first_available_channel_inf(std::chrono::milliseconds ticks,
	                                                   const object <Mix_Chunk>& effect) {
	...
	mixer::fade_in(unsigned int loops, std::chrono::milliseconds ticks, std::chrono::milliseconds pos, const object <
		...
		using namespace std::chrono_literals;
		double fp_seconds = static_cast <double>(pos / 1.0s);
		SAFE_SDL_CALL(Mix_FadeInMusicPos, mus.const_handle(), static_cast<int>(loops), static_cast<int>(ticks.count()),
		              fp_seconds);
	}

	void mixer::fade_in(unsigned loops, std::chrono::milliseconds ticks, std::chrono::seconds pos,
	                    const object <Mix_Music>& mus) {
		SAFE_SDL_CALL(Mix_FadeInMusicPos, mus.const_handle(), static_cast<int>(loops), static_cast<int>(ticks.count()),
		              static_cast<double>(pos.count()));
	}
	...
	void mixer::fade_in_inf(std::chrono::milliseconds ticks, std::chrono::milliseconds pos,
	                        const object <Mix_Music>& mus) {
		using namespace std::chrono_literals;
		double fp_seconds = static_cast <double>(pos / 1.0s);
	...
	void mixer::fade_in_inf(std::chrono::milliseconds ticks, std::chrono::seconds pos, const object <Mix_Music>& mus) {
		SAFE_SDL_CALL(Mix_FadeInMusicPos, mus.const_handle(), -1, static_cast<int>(ticks.count()),
		              static_cast<double>(pos.count()));
	}
	...
	void mixer::fade_in(audio_channel_id_t chan, unsigned loops, std::chrono::milliseconds effect_duration,
	                    std::chrono::milliseconds ticks, const object <Mix_Chunk>& effect) {
		SAFE_SDL_CALL(Mix_FadeInChannelTimed, static_cast<int>(chan.value_of()), effect.const_handle(),
		              static_cast<int>(loops), static_cast<int>(ticks.count()),
		              static_cast<int>(effect_duration.count()));
	}

	void mixer::fade_in(audio_channel_id_t chan, std::chrono::milliseconds effect_duration,
	                    std::chrono::milliseconds ticks, const object <Mix_Chunk>& effect) {
		SAFE_SDL_CALL(Mix_FadeInChannelTimed, static_cast<int>(chan.value_of()), effect.const_handle(), 0,
		              static_cast<int>(ticks.count()), static_cast<int>(effect_duration.count()));
	}

	void mixer::fade_in_inf(audio_channel_id_t chan, std::chrono::milliseconds effect_duration,
	                        std::chrono::milliseconds ticks, const object <Mix_Chunk>& effect) {
		SAFE_SDL_CALL(Mix_FadeInChannelTimed, static_cast<int>(chan.value_of()), effect.const_handle(), -1,
		              static_cast<int>(ticks.count()), static_cast<int>(effect_duration.count()));
	}

	bool mixer::fade_in_on_first_available_channel(unsigned loops, std::chrono::milliseconds effect_duration,
	                                               std::chrono::milliseconds ticks, const object <Mix_Chunk>& effect) {
		return -1 != Mix_FadeInChannelTimed(-1, effect.const_handle(), static_cast <int>(loops),
		                                    static_cast <int>(ticks.count()),
		                                    static_cast <int>(effect_duration.count()));
	}

	bool mixer::fade_in_on_first_available_channel(std::chrono::milliseconds effect_duration,
	                                               std::chrono::milliseconds ticks, const object <Mix_Chunk>& effect) {
		return -1 != Mix_FadeInChannelTimed(-1, effect.const_handle(), 0, static_cast <int>(ticks.count()),
		                                    static_cast <int>(effect_duration.count()));
	}

	bool mixer::fade_in_on_first_available_channel_inf(std::chrono::milliseconds effect_duration,
	                                                   std::chrono::milliseconds ticks,
	                                                   const object <Mix_Chunk>& effect) {
		return -1 != Mix_FadeInChannelTimed(-1, effect.const_handle(), -1, static_cast <int>(ticks.count()),
		                                    static_cast <int>(effect_duration.count()));
	}
	...
	std::vector <std::string> mixer::get_sound_fonts() {
		...
		bsw::string_tokenizer tokenizer(
			fonts, ";", bsw::string_tokenizer::TOK_IGNORE_EMPTY | bsw::string_tokenizer::TOK_TRIM);
