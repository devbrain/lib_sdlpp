...
/**
 * @file enclose.hh
 * @brief Tight axis-aligned bounding box (@ref simplex::collide::aabb) enclosure of the
 *        collision shape value types.
 *
 * Provides a small set of overloaded @c enclose() free functions, one per shape, each returning
 * the smallest @ref simplex::collide::aabb that fully contains the shape. These compute the
 ...
 * exactly (modulo floating-point representation). The @ref simplex::collide::aabb overload is the
 * identity, present so generic code can call @c enclose() uniformly regardless of shape type.
 *
 * @note No @c enclose() overload is provided for @ref simplex::collide::triangle here.
