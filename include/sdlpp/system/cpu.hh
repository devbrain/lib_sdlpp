...
#undef d_evalCap
	...
	template<typename T>
	static constexpr const decltype(detail::s_vals_of_cpu_capability)&
	values(std::enable_if_t <std::is_same_v <cpu::capability, T>>* = nullptr) {
		return detail::s_vals_of_cpu_capability;
	}

	template<typename T>
	static constexpr auto
	begin(std::enable_if_t <std::is_same_v <cpu::capability, T>>* = nullptr) {
		return detail::s_vals_of_cpu_capability.begin();
	}

	template<typename T>
	static constexpr auto
	end(std::enable_if_t <std::is_same_v <cpu::capability, T>>* = nullptr) {
