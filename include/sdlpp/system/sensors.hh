...
namespace neutrino::sdl {
	namespace detail {
		struct _sensor_device_;
		struct _sensor_device_id_;
	}

	using sensor_device_t = strong::type <std::size_t,
	                                      detail::_sensor_device_,
	                                      strong::bicrementable,
	                                      strong::ordered,
	                                      strong::equality,
	                                      strong::ostreamable>;
	using sensor_device_id_t = strong::type <SDL_SensorID,
	                                         detail::_sensor_device_id_,
	                                         strong::bicrementable,
	                                         strong::ordered,
	                                         strong::equality,
	                                         strong::ostreamable>;
	...
	class sensor : public object <SDL_Sensor> {
		public:
			...
			static sensor_device_id_t get_id(sensor_device_t idx) {
				return sensor_device_id_t{
					SAFE_SDL_CALL(SDL_SensorGetDeviceInstanceID, static_cast<int>(idx.value_of ()))
				};
			}
			...
			template<class T>
			std::vector <T>
			get_data(std::size_t num_values,
			         typename std::enable_if <std::is_same_v <T, float>>::type* = nullptr) const;

			template<class T>
			std::vector <T> get_data(std::size_t num_values, typename std::enable_if <std::is_same_v <T,
	...
	template<typename T>
	static constexpr const decltype (detail::s_vals_of_sensor_type)&
	values(std::enable_if_t <std::is_same_v <sensor::type, T>>* = nullptr) {
		return detail::s_vals_of_sensor_type;
	}

	template<typename T>
	static constexpr auto
	begin(std::enable_if_t <std::is_same_v <sensor::type, T>>* = nullptr) {
		return detail::s_vals_of_sensor_type.begin();
	}

	template<typename T>
	static constexpr auto
	end(std::enable_if_t <std::is_same_v <sensor::type, T>>* = nullptr) {
	...
	template<class T>
	std::vector <T>
	sensor::get_data(std::size_t num_values, std::enable_if_t <std::is_same_v <T, float>>*) const {
		std::vector <T> t(num_values);
		get_data(static_cast <float*>(t.data()), num_values);
		return t;
	}

	template<class T>
	std::vector <T>
	sensor::get_data(std::size_t num_values, std::enable_if_t <std::is_same_v <T, timed_data_t>>*) const {
