...
namespace neutrino::sdl {
	...
	template<typename T>
	[[nodiscard]]
	std::vector <SDL_Event> peek_events(std::size_t num_events, event_type from, event_type to,
	                                    std::enable_if_t <std::is_same_v <T, SDL_Event>>* = nullptr) {
		std::vector <SDL_Event> out(num_events);
		int rc = SAFE_SDL_CALL(SDL_PeepEvents, out.data(), static_cast<int>(num_events), SDL_PEEKEVENT,
	...
	template<typename T>
	[[nodiscard]]
	std::vector <SDL_Event> peek_events(std::size_t num_events,
	                                    std::enable_if_t <std::is_same_v <T, SDL_Event>>* = nullptr) {
		std::vector <SDL_Event> out(num_events);
		int rc = SAFE_SDL_CALL(SDL_PeepEvents, out.data(), static_cast<int>(num_events), SDL_PEEKEVENT, SDL_FIRSTEVENT,
		                       SDL_LASTEVENT);
		out.resize(static_cast <std::size_t>(rc));
		return out;
	}
	...
	template<typename T>
	[[nodiscard]]
	std::vector <events::event_t> peek_events(std::size_t num_events,
	                                          std::enable_if_t <std::is_same_v <T, events::event_t>>* = nullptr) {
		auto e = peek_events <SDL_Event>(num_events);
		std::vector <events::event_t> out(e.size());
		std::transform(e.begin(), e.end(), out.begin(), [](const auto& ev) { return map_event(ev); });
		return out;
	}

	template<typename T>
	[[nodiscard]]
	std::vector <events::event_t> peek_events(std::size_t num_events, event_type from, event_type to,
	                                          std::enable_if_t <std::is_same_v <T, events::event_t>>* = nullptr) {
		auto e = peek_events <SDL_Event>(num_events, from, to);
		std::vector <events::event_t> out(e.size());
		std::transform(e.begin(), e.end(), out.begin(), [](const auto& ev) { return map_event(ev); });
		return out;
	}
	...
	template<typename T>
	[[nodiscard]]
	std::vector <SDL_Event> remove_events(std::size_t num_events, event_type from, event_type to,
	                                      std::enable_if_t <std::is_same_v <T, SDL_Event>>* = nullptr) {
		std::vector <SDL_Event> out(num_events);
		int rc = SAFE_SDL_CALL(SDL_PeepEvents, out.data(), static_cast<int>(num_events), SDL_GETEVENT,
		                       static_cast<uint32_t>(from), static_cast<uint32_t>(to));
		out.resize(static_cast <std::size_t>(rc));
		return out;
	}
	...
	template<typename T>
	[[nodiscard]]
	std::vector <SDL_Event> remove_events(std::size_t num_events,
	                                      std::enable_if_t <std::is_same_v <T, SDL_Event>>* = nullptr) {
		std::vector <SDL_Event> out(num_events);
		int rc = SAFE_SDL_CALL(SDL_PeepEvents, out.data(), static_cast<int>(num_events), SDL_GETEVENT, SDL_FIRSTEVENT,
		                       SDL_LASTEVENT);
		out.resize(static_cast <std::size_t>(rc));
		return out;
	}
	...
	template<typename T>
	[[nodiscard]]
	std::vector <events::event_t> remove_events(std::size_t num_events,
	                                            std::enable_if_t <std::is_same_v <T, events::event_t>>* = nullptr) {
		auto e = remove_events <SDL_Event>(num_events);
		std::vector <events::event_t> out(e.size());
	...
	template<typename T>
	[[nodiscard]]
	std::vector <events::event_t> remove_events(std::size_t num_events, event_type from, event_type to,
	                                            std::enable_if_t <std::is_same_v <T, events::event_t>>* = nullptr) {
		auto e = remove_events <SDL_Event>(num_events, from, to);
		std::vector <events::event_t> out(e.size());
		std::transform(e.begin(), e.end(), out.begin(), [](const auto& ev) { return map_event(ev); });
		return out;
	}
	...
	void add_events(const std::vector <SDL_Event>& events) {
		SAFE_SDL_CALL(SDL_PeepEvents, const_cast<std::vector<SDL_Event>&>(events).data(),
		              static_cast<int>(events.size()), SDL_ADDEVENT, SDL_FIRSTEVENT, SDL_LASTEVENT);
	}
	...
	void push_event(const events::user& u) {
		...
		auto& [type, timestamp, windowID, code, data1, data2] = sdl_ev.user;
	...
	inline
	std::optional <events::event_t> wait_for_event(const std::chrono::milliseconds ms) {
		SDL_Event ev;
